...
class LocationController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        $listLocation = Location::query()->with('category') ;
        ...
        $data = [
            ...
            'categories'  => LocationCategory::where('status', 'publish')->get()->toTree(),
            'breadcrumbs' => [
                [
                    'name' => __('Location'),
    ...
    public function getLocationCategory(Request $request)
    {
        ...
        if($pre_selected && $selected){
            ...
            {
                $items = LocationCategory::select('id', 'name as text')->whereIn('id',$selected)->take(50)->get();
                return response()->json([
                    'items'=>$items
                ]);
            }else{
                $item = LocationCategory::find($selected);
            }
            if(empty($item)){
                return response()->json([
                    'text'=>''
        ...
        $q = $request->query('q');
        $query = LocationCategory::select('id', 'name as text')->where("status","publish");
        if ($q) {
