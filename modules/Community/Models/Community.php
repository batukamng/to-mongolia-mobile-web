...
class Community extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;

    protected $table                              = 'bravo_communitys';
    public    $checkout_booking_detail_file       = 'Community::frontend/booking/detail';
    public    $checkout_booking_detail_modal_file = 'Community::frontend/booking/detail-modal';
    public    $set_paid_modal_file                = 'Community::frontend/booking/set-paid-modal';
    public    $email_new_booking_file             = 'Community::emails.new_booking_detail';
    public    $type                               = 'community';
    public    $availabilityClass                  = CommunityDate::class;
    ...
    protected $communityMetaClass;
    protected $communityDateClass;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->communityMetaClass = CommunityMeta::class;
        $this->communityDateClass = CommunityDate::class;
     ...
     */
    public function category_community()
    {
        return $this->hasOne("Modules\Community\Models\CommunityCategory", "id", 'category_id')->with(['translations']);
    }
    ...
    {
        $meta['seo_title'] = __("Where would you like to go");
        if (!empty($title = setting_item_with_lang("community_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("community_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("community_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("community_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("community_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("community_page_list_seo_share");
    ...
    {
        return $this->hasOne($this->communityMetaClass, "community_id");
    }
     ...
     */
    public function community_term()
    {
        return $this->hasMany($this->communityTermClass, "community_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('community.community_route_prefix') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    public static function getLinkForPageSearch($locale = false, $param = [])
    {
        ...
        return url(app_get_locale(false, false, '/') . config('community.community_route_prefix') . "?" . http_build_query($param));
    }
    ...
    {
        return route('community.admin.edit',['id'=>$this->id]);
    }
    ...
    {
        $meta = $this->communityMetaClass::where('community_id', $this->id)->first();
        if (!$meta) {
            $meta = new $this->communityMetaClass();
            $meta->community_id = $this->id;
        }
        $arg = $request->input();
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('community_booking_buyer_fees'))) {
    ...
    {
        $datesRaw = $this->communityDateClass::getDatesInRanges($start_date, $this->id);
        $dates = [
            'base_price'   => null,
            'person_types' => null,
        ];
    ...
    {
        $communityDate = $this->communityDateClass::where('target_id', $this->id)->where('start_date', $start_date)->where('active', 1)->first();
        ...
        $maxGuests = !empty($communityDate->max_guests) ? $communityDate->max_guests : $this->max_people;
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if($this->isFixedDate()){
            if(Carbon::parse($request->start_date.' 00:00:00') <> $this->start_date){
                return $this->sendError(__("This community is not available at selected dates"));
            }
        }else{
            ...
            if ($meta) {
                ...
                if ($meta->enable_open_hours) {
                    ...
                    if (!isset($open_hours[$nDate]) or empty($open_hours[$nDate]['enable'])) {
                        return $this->sendError(__("This community is not open on your selected day"));
                    }
                }
            }
    ...
    public function isAvailableInRanges($start_date){
        ...
        {
            $notAvailableDates = $this->communityDateClass::query()->where([
            ...
            $availableDates = $this->communityDateClass::query()->where([
                ['start_date','>=',$start_date],
                ['end_date','<=',$start_date],
                ['active','=',1],
                ['target_id','=',$this->id],
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('community_booking_buyer_fees');
        if (!empty($list_fees)) {
    ...
    {
        return setting_item("community_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("community_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("community_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("community_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("community_review_stats", []))) {
    ...
    public function getReviewDataAttribute()
    {
        ...
        $dataTotalReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $this->id)->where('object_model', "community")->where("status", "approved")->first();
     ...
     * Get Score Review
     *
     * Using for loop community
     */
    public function getScoreReview()
    {
        $community_id = $this->id;
        $list_score = Cache::rememberForever('review_' . $this->type . '_' . $community_id, function () use ($community_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $community_id)->where('object_model', "community")->where("status", "approved")->first();
            return [
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            $number = parent::join('bravo_locations', function ($join) use ($location) {
                $join->on('bravo_locations.id', '=', 'bravo_communitys.location_id')->where('bravo_locations._lft', '>=', $location->_lft)->where('bravo_locations._rgt', '<=', $location->_rgt);
            })->where("bravo_communitys.status", "publish")->with(['translations'])->count("bravo_communitys.id");
        }
        if (empty($number))
            return false;
        if ($number > 1) {
            return __(":number Communitys", ['number' => $number]);
        }
        return __(":number Community", ['number' => $number]);
    }

    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])
            ->where('object_id', $this->id)
            ->where('object_model', 'community')
            ->where("status", "approved")
            ->orderBy("id", "desc")
            ->with('author')
            ->paginate(setting_item('community_review_number_per_page', 5));
    }
    ...
    public function getBookingsInRange($from, $to)
    {
        ...
        $query->where('object_model', 'community');
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        if (empty($old))
            return false;
        $selected_terms = $old->community_term->pluck('term_id');
        ...
        //Language
        $langs = $this->communityTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
        ...
        //Meta
        $metaCommunity = $this->communityMetaClass::where('community_id', $old->id)->first();
        if (!empty($metaCommunity)) {
            $metaCommunityNew = $metaCommunity->replicate();
            $metaCommunityNew->community_id = $new->id;
            $metaCommunityNew->save();
        }
    }
    ...
    {
        return setting_item('community_disable') == false;
    }

    public function isDepositEnable()
    {
        return (setting_item('community_deposit_enable') and setting_item('community_deposit_amount'));
    }

    public function getDepositAmount()
    {
        return setting_item('community_deposit_amount');
    }

    public function getDepositType()
    {
        return setting_item('community_deposit_type');
    }

    public function getDepositFomular()
    {
        return setting_item('community_deposit_fomular', 'default');
    }

    public static function isEnableEnquiry()
    {
        if (!empty(setting_item('booking_enquiry_for_community'))) {
            return true;
        }
        return false;
    }
    ...
    {
        $check = setting_item('booking_enquiry_for_community');
        if (!empty($check) and setting_item('booking_enquiry_type_community') == "booking_and_enquiry") {
            return true;
        }
        return false;
    }
    ...
    public static function getBookingEnquiryType()
    {
        $check = setting_item('booking_enquiry_for_community');
        if (!empty($check)) {
            if (setting_item('booking_enquiry_type_community') == "only_enquiry") {
                return "enquiry";
            }
        }
        return "book";
    ...
    {
        $model_Community = parent::query()->select("bravo_communitys.*");
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_communitys.sale_price,0) > 0 and bravo_communitys.sale_price >= ? ) OR (IFNULL(bravo_communitys.sale_price,0) <= 0 and bravo_communitys.price >= ?) )
								AND ( (IFNULL(bravo_communitys.sale_price,0) > 0 and bravo_communitys.sale_price <= ? ) OR (IFNULL(bravo_communitys.sale_price,0) <= 0 and bravo_communitys.price <= ?) )";
            $model_Community->WhereRaw($raw_sql_min_max, [
                $pri_from,
                $pri_from,
                $pri_to,
                $pri_to
        ...
        if (!empty($category_ids = $request->query('cat_id')) and !empty($category_ids[0])) {
            ...
            if (!empty($list_cat)) {
                ...
                $model_Community->join('bravo_community_category', function ($join) use ($sql_where_join,$params) {
                        $join->on('bravo_community_category.id', '=', 'bravo_communitys.category_id')->WhereRaw($sql_where_join,$params);
                    });
            }
        }
        $terms = $request->query('terms');
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_Community->join('bravo_community_term as tt', 'tt.community_id', "bravo_communitys.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number) {
                $where_review_score[] = " ( bravo_communitys.review_score >= ? AND bravo_communitys.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_Community->WhereRaw($sql_where_review_score,$params);
        }
        if (!empty($service_name = $request->query("service_name"))) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $model_Community->leftJoin('bravo_community_translations', function ($join) {
                    $join->on('bravo_communitys.id', '=', 'bravo_community_translations.origin_id');
                });
                $model_Community->where('bravo_community_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $model_Community->where('bravo_communitys.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn')) and !empty($request->query('map_place'))){
            //			3959 - Miles(dặm), 6371 - Kilometers
                $distance  = setting_item('community_location_radius_value',0);
                if(!empty($distance) and setting_item('community_location_search_style')=='autocompletePlace'){
                    $distanceType = setting_item('community_location_radius_type',3959);
                    ...
                    $model_Community->whereRaw($string,[$distanceType,$lat,$lgn,$lat,$distance]);
                }
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
            $model_Community->orderByRaw("POW((bravo_communitys.map_lng-?),2) + POW((bravo_communitys.map_lat-?),2)",[$lgn,$lat]);
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_communitys.sale_price, 0 ) > 0 THEN bravo_communitys.sale_price ELSE bravo_communitys.price END AS tmp_min_price";
                $model_Community->selectRaw($raw_sql);
                $model_Community->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_communitys.sale_price, 0 ) > 0 THEN bravo_communitys.sale_price ELSE bravo_communitys.price END AS tmp_min_price";
                $model_Community->selectRaw($raw_sql);
                $model_Community->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
                $model_Community->orderBy("review_score", "desc");
                break;
            default:
                $model_Community->orderBy("is_featured", "desc");
                $model_Community->orderBy("id", "desc");
        }
        ...
        if(!empty($request->query('limit'))){
            ...
            $limit = !empty(setting_item("community_page_limit_item"))? setting_item("community_page_limit_item") : 9;
        }
        return $model_Community->with([
            'location',
            'hasWishList',
            'translations'
        ])->paginate($limit);
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            $data['category'] = CommunityCategory::selectRaw("id,name")->find($this->category_id) ?? null;
            ...
            $data['booking_fee'] = setting_item_array('community_booking_buyer_fees');
            ...
            $data['terms'] = Terms::getTermsByIdForAPI($this->community_term->pluck('term_id'));
        } else {
    ...
    {
        return "\Modules\Community\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        $category = CommunityCategory::selectRaw("id,name,slug")->where('status', 'publish')->with(['translations'])->get()->toTree();
        return [
            [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "4",
                "data" => Attributes::getAllAttributesForApi("community")
            ]
        ];
    }
    ...
    {
        $search_fields = setting_item_array('community_search_fields');
