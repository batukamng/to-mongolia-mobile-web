...
use Modules\Community\Models\CommunityDate;
...
class AvailabilityController extends FrontendController
{
    ...
    protected $indexView = 'Community::frontend.user.availability';
    ...
    public function index(Request $request)
    {
        ...
        $page_title = __('Communitys Availability');
    ...
    public function loadDates(Request $request)
    {
        ...
        $community = $this->communityClass::find($request->query('id'));
        if (empty($community)) {
            return $this->sendError(__('Community not found'));
        }
        $lang = app()->getLocale();
        $is_single = $request->query('for_single');
        $query = $this->communityDateClass::query();
        ...
        foreach ($period as $dt){
            ...
            $date = [
                'id'           => rand(0, 999),
                'active'       => 0,
                'price'        => (!empty($community->sale_price) and $community->sale_price > 0 and $community->sale_price < $community->price) ? $community->sale_price : $community->price,
                'is_default'   => true,
                'textColor'    => '#2791fe',
            ];
            if (!$is_single) {
            ...
            $date['max_guests'] = $community->max_people;
            $date['title_origin'] = $date['price_html'];
            $date['title'] = $date['event'] = $date['price_html'] . "<br>". __('Max guests: ') . $community->max_people;
            $date['start'] = $date['end'] = date('Y-m-d', $i);
            if ($community->default_state) {
                $date['active'] = 1;
            } else {
            ...
            if ($request->input('for_single')) {
                if (empty(!$community->max_people) and $community->max_people < 1) {
                    $date['active'] = 0;
                }
            }
            if (!empty($community->meta->enable_person_types) and $community->meta->enable_person_types == 1) {
                $date['person_types'] = $community->meta->person_types;
            ...
            // Open Hours
            if (!empty($community->meta->enable_open_hours) and $community->meta->enable_open_hours == 1) {
                $open_hours = $community->meta->open_hours;
                $nDate = date('N', $i);
        ...
        if (!empty($rows)) {
            foreach ($rows as $row) {
                ...
                if (!empty($community->meta->enable_person_types) and $community->meta->enable_person_types == 1) {
                    $list_person_types = $community->meta->person_types;
        ...
        $bookings = $this->bookingClass::getBookingInRanges($community->id, $community->type, $request->query('start'), $request->query('end'));
        if (!empty($bookings)) {
        ...
        if (!empty($community->ical_import_url)) {
            $startDate = $request->query('start');
            $endDate = $request->query('end');
            $timezone = setting_item('site_timezone', config('app.timezone'));
            try {
                $icalevents = new Ical($community->ical_import_url, [
    ...
    public function store(Request $request)
    {
        ...
        $community = $this->communityClass::find($request->input('target_id'));
        $target_id = $request->input('target_id');
        if (empty($community)) {
            return $this->sendError(__('Community not found'));
        }
        if (!$this->hasPermission('community_manage_others')) {
            if ($community->create_user != Auth::id()) {
...
//        for ($i = strtotime($request->input('start_date')); $i <= strtotime($request->input('end_date')); $i += DAY_IN_SECONDS) {
        ...
        foreach ($period as $dt){
            $date = $this->communityDateClass::where('start_date', $dt->format('Y-m-d'))->where('target_id', $target_id)->first();
            if (empty($date)) {
                $date = new $this->communityDateClass();
