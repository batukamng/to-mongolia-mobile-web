...
@section('content')
    <form action="{{route('community.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->title : __('Add new community')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url(config('community.community_route_prefix') ) }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("View Community")}}</a>
                    @endif
                </div>
            </div>
            ...
            <div class="lang-content-box">
                <div class="row">
                    <div class="col-md-9">
                        @include('Community::admin/community/community-content')
                        @include('Community::admin/community/community-location')
                        @include('Hotel::admin.hotel.surrounding')

                    @if(is_default_lang())
                            @include('Community::admin/community/pricing')
                            @include('Community::admin/community/availability')
                        @endif
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            <div class="panel">
                                <div class="panel-title"><strong>{{__("Community Featured")}}</strong></div>
                                <div class="panel-body">
                                    <div class="form-group">
                            ...
                            @include('Community::admin/community/ical')
                        @endif
                    </div>
                </div>
            </div>
