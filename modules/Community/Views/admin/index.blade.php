...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recovery') : __("All Community")}}</h1>
            <div class="title-actions">
                @if(empty($recovery))
                <a href="{{route('community.admin.create')}}" class="btn btn-primary">{{__("Add new community")}}</a>
                @endif
            </div>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{route('community.admin.bulkEdit')}}" class="filter-form filter-form-left d-flex justify-content-start">
                        {{csrf_field()}}
            ...
            <div class="col-left dropdown">
                <form method="get" action="{{ !empty($recovery) ? route('community.admin.recovery') : route('community.admin.index')}}" class="filter-form filter-form-right d-flex justify-content-end flex-column flex-sm-row" role="search">
                    <input type="text" name="s" value="{{ Request()->s }}" placeholder="{{__('Search by name')}}" class="form-control">
                    @if(!empty($rows) and $community_manage_others)
                            ...
                            <div class="dropdown-menu px-3 py-3 dropdown-menu-right" aria-labelledby="dropdown_filters">
                                <div class="mb-3">
                                    ...
                                    <select name="cate_id" class="form-control">
                                        ...
                                        @foreach($community_categories as $category)
                            ...
                            @if($rows->total() > 0)
                                @foreach($rows as $row)
                                    <tr class="{{$row->status}}">
                                        ...
                                        <td class="title">
                                            <a href="{{route('community.admin.edit',['id'=>$row->id])}}">{{$row->title}}</a>
                                        ...
                                        <td>
                                            <a href="{{route('community.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
