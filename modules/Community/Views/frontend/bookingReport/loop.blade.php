<tr>
    ...
    <td width="2%">
        ...
        @if(!empty(setting_item("community_allow_vendor_can_change_their_booking_status")))
            ...
            <div class="dropdown-menu">
                @if(!empty($statues))
                    @foreach($statues as $status)
                        <a href="{{ route("community.vendor.booking_report.bulk_edit" , ['id'=>$booking->id , 'status'=>$status]) }}">
        ...
        @endif
        @if(!empty(setting_item("community_allow_vendor_can_change_paid_amount")))
