...
class FormSearchCommunity extends BaseBlock
{
    ...
    {
        return __('Community: Form Search');
    }

    public function content($model = [])
    {
        $limit_location = 15;
        if( empty(setting_item("community_location_search_style")) or setting_item("community_location_search_style") == "normal" ){
            $limit_location = 1000;
        }
        $data = [
            'community_location' => Location::where("status","publish")->limit($limit_location)->with(['translations'])->get()->toTree(),
            'bg_image_url'  => '',
        ];
        ...
        return view('Community::frontend.blocks.form-search-community.index', $data);
    }
