...
class RunUpdater
{
    ...
    public function updateTo110()
    {
        ...
        $vendor->givePermissionTo('community_view');
        $vendor->givePermissionTo('community_create');
        $vendor->givePermissionTo('community_update');
        $vendor->givePermissionTo('community_delete');
    ...
    public function updateTo250()
    {
        ...
        // Pickup
        // Permission::findOrCreate('pickup_view');
        // Permission::findOrCreate('pickup_create');
        // Permission::findOrCreate('pickup_update');
        // Permission::findOrCreate('pickup_delete');
        // Permission::findOrCreate('pickup_manage_others');
        // Permission::findOrCreate('pickup_manage_attributes');
        // Vendor
        // $vendor = Role::findOrCreate('vendor');
        // $vendor->givePermissionTo('pickup_create');
        // $vendor->givePermissionTo('pickup_view');
        // $vendor->givePermissionTo('pickup_update');
        // $vendor->givePermissionTo('pickup_delete');
        // // Admin
        // $role = Role::findOrCreate('administrator');
        // $role->givePermissionTo('pickup_view');
        // $role->givePermissionTo('pickup_create');
        // $role->givePermissionTo('pickup_update');
        // $role->givePermissionTo('pickup_delete');
        // $role->givePermissionTo('pickup_manage_others');
        // $role->givePermissionTo('pickup_manage_attributes');
    ...
    public function updateTo151()
    {
        ...
        Schema::table(Community::getTableName(), function (Blueprint $table) {
            if (!Schema::hasColumn(Community::getTableName(), 'ical_import_url')) {
                $table->string('ical_import_url')->nullable();
            }
        });
        ...
        // Schema::table(Pickup::getTableName(), function (Blueprint $table) {
        //     if (!Schema::hasColumn(Pickup::getTableName(), 'ical_import_url')) {
        //         $table->string('ical_import_url')->nullable();
        //     }
        // });

        // Schema::table(PickupTranslation::getTableName(), function (Blueprint $table) {
        //     if (Schema::hasColumn(PickupTranslation::getTableName(), 'extra_price')) {
        //         $table->dropColumn('extra_price');
        //     }
        // });
    ...
    public function updateTo170()
    {
        ...
        if (empty(setting_item('community_map_search_fields'))) {
            DB::table('core_settings')->insert(
                [
        ...
        if (empty(setting_item('community_search_fields'))) {
            DB::table('core_settings')->insert(
                [
        ...
        // if (empty(setting_item('pickup_search_fields'))) {
        //     DB::table('core_settings')->insert(
        //         [
        //             'name'  => 'pickup_search_fields',
        //             'val'   => '[{"title":"Location","field":"location","size":"6","position":"1"},{"title":"From - To","field":"date","size":"6","position":"2"}]',
        //             'group' => 'pickup'
        //         ]
        //     );
...
//        });
    ...
    public function updateTo190()
    {
        ...
        Schema::table(Community::getTableName(), function (Blueprint $table) {
            if (!Schema::hasColumn(Community::getTableName(), 'service_fee')) {
                $table->tinyInteger('enable_service_fee')->nullable();
                $table->text('service_fee')->nullable();
            }
        });
        ...
        // Schema::table(Pickup::getTableName(), function (Blueprint $table) {
        //     if (!Schema::hasColumn(Pickup::getTableName(), 'service_fee')) {
        //         $table->tinyInteger('enable_service_fee')->nullable();
        //         $table->text('service_fee')->nullable();
        //     }
        // });
        ...
        Schema::table(Community::getTableName(), function (Blueprint $table) {
            if (!Schema::hasColumn(Community::getTableName(), 'surrounding')) {
                $table->text('surrounding')->nullable();
            }
        });
        Schema::table(CommunityTranslation::getTableName(), function (Blueprint $table) {
            if (!Schema::hasColumn(CommunityTranslation::getTableName(), 'surrounding')) {
                $table->text('surrounding')->nullable();
            }
        });
    ...
    public function updateTo200()
    {
        ...
        Schema::table('bravo_communitys', function (Blueprint $table) {
            if (!Schema::hasColumn('bravo_communitys', 'min_day_before_booking')) {
                $table->integer('min_day_before_booking')->nullable();
            }
        });
            ...
            foreach ($all as $item) {
                switch ($item->object_model) {
                    case "tour":
                        ...
                        break;
                    case "community":
                        $community = Community::find($item->object_id);
                        if ($community) {
                            $item->vendor_id = $community->create_user;
                            $item->save();
                        }
                        break;
                    case "space":
